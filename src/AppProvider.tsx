...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  getTaskList: async (id: string) => {
    const response = await axios.get<TaskList>(
      `http://localhost:8081/api/task-lists/${id}`,
      {
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem('token')}`
  ...
  getTask: async (taskListId: string, taskId: string) => {
    const response = await axios.get<Task>(
      `http://localhost:8081/api/task-lists/${taskListId}/tasks/${taskId}`,
      {
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem('token')}`
  ...
  deleteTaskList: async (id) => {
    await axios.delete(`http://localhost:8081/api/task-lists/${id}`, {
      headers: { 
        "Content-Type": "application/json",
        "Authorization": `Bearer ${localStorage.getItem('token')}`
      }
  ...
  fetchTasks: async (taskListId) => {
    const response = await axios.get<Task[]>(
      `http://localhost:8081/api/task-lists/${taskListId}/tasks`,
      {
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem('token')}`
